...
| **凭证死了没人喊** | OAuth 令牌失效当天,例行照常跑完七步、一句异常没报 —— 碰平台的步骤大多「安静地什么也没做」;我自己的 `tail -30` 又把错误截掉了 | 凭证检查进 [0/7] 前置门,失效即拦停并给出修复命令。⚠ Google OAuth 应用处于 **Testing** 时 refresh token 只活 7 天;2026-08-21 已改为 **In production**,不再定期过期 |
| **门写成了永远放行** | 前置门的凭证守卫第一版写 `return 1` —— 而该函数契约是「True=健康」,1 是真值,等于没有门 | 守卫返回布尔;加了门之后必须**看它真的拦下一次**再收工 |
