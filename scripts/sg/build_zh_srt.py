...
#
# 「别」只在作「不要」讲时才是否定（别怕／别走）。分别、特别、区别、告别
# 里的「别」不表否定，此前一律误报——2026-08-11 三期中文稿有两条卡在
# 「分别为圣」上。软门禁误报多了就没人看了，跟只数条数的假 PASS 是同一种病。
ZH_NEG = re.compile(r"不|没|沒|无|無|非|未|勿|"
                    r"(?<![分区區特差个個级級派性类類识識辨告])[别別]")
